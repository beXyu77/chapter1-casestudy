...
public class InventoryPane extends ScrollPane {
            ...
            for (int i = 0; i < equipmentArray.size(); i++) {
                ...
                imageViewList[i].setOnDragDetected(new EventHandler<MouseEvent>() {
                    public void handle(MouseEvent event) {
                        onDragDetected(event, equipmentArray.get(finall), imageViewList[finall]);
                    }
                });
                ...
                imageViewList[i].setOnDragDropped(new EventHandler<DragEvent>() {
                    @Override
                    public void handle(DragEvent event) {
                        BasedEquipment draggedItem = Launcher.getDraggedItem();
                        if (draggedItem != null) {
                            Launcher.addItemToInventory(draggedItem);
                        }
                    }
                });
            }
    ...
    public void addItem(BasedEquipment item) {
        if (equipmentArray != null) {
            equipmentArray.add(item);
            drawPane(equipmentArray);
        }
    }
}
